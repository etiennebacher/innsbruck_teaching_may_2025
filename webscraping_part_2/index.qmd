---
title: "Webscraping -- Part 2"
subtitle: "Dynamic webscraping"
...
-   if yes, is there a package?
    ...
    -   if no, build the API queries yourself with [`httr2`](https://httr2.r-lib.org/)
...
```{r}
remote_driver$findElement(using = "css", value = ".my-button")$clickElement()
```
...
Install the package [`rJavaEnv`](https://www.ekotov.pro/rJavaEnv/), and then run `rJavaEnv::java_quick_install(version = 21)`.
...
[Open website](https://www.acervodigital.museudaimigracao.org.br/livros.php){.external target="_blank"}
...
Not really and we still need RSelenium to change pages (and this is just training
anyway).
:::
:::
...
```{r}
remote_driver$findElement("id", "2")$highlightElement()
```
...
- Python also has a library called [`playwright`](https://playwright.dev/python/) (never used myself).
...
Using POST requests instead of `RSelenium`: [https://www.etiennebacher.com/posts/2023-05-09-making-post-requests-with-r/](https://www.etiennebacher.com/posts/2023-05-09-making-post-requests-with-r/)
